...
class BipedWF(BaseTask):
    ...
    def _randomize_arm_pose(self, env_ids):
        ...
        # 随机化机械臂方向（如果启用）
        if self.cfg.domain_rand.randomize_arm_orientation and len(arm_joint_indices) > 0:
            # 为每个关节生成随机角度偏移
            ...
            for i, env_id in enumerate(env_ids):
                for j, joint_idx in enumerate(arm_joint_indices):
                    # 获取当前关节位置并添加随机偏移
                    current_pos = self.dof_pos[env_id, joint_idx]
                    self.dof_pos[env_id, joint_idx] = current_pos + orientation_noise[i, j]
